...
const PIDUpload = () => {
  ...
  // â”€â”€ SOFT-CODED: 8-pass analysis stage tracker (mirrors services.py pass count) â”€â”€
  const PASS_STAGES = [
    { pass: 1, label: 'OCR Extraction',       icon: 'ðŸ“„' },
    { pass: 2, label: 'Reference Processing', icon: 'ðŸ“š' },
    { pass: 3, label: 'Vision Analysis',      icon: 'ðŸ‘ï¸' },
    { pass: 4, label: 'Cross-Validation',     icon: 'ðŸ”—' },
    { pass: 5, label: 'Second Review',        icon: 'ðŸ”' },
    { pass: 6, label: 'Eng. Compliance',      icon: 'âš™ï¸' },
    { pass: 7, label: 'Line Size Check',      icon: 'ðŸ“' },
    { pass: 8, label: 'Smart QC',             icon: 'âœ…' },
  ];

  // SOFT-CODED: Engineering facts displayed during the analysis wait â€“ keeps the user engaged
  const OIL_GAS_FACTS = [
    { icon: 'âš™ï¸', text: 'P&ID drawings are the single source of truth for process plant construction, operation, and maintenance documentation.' },
    { icon: 'ðŸ›¡ï¸', text: 'PSV set pressure must never exceed equipment design pressure â€” a fundamental requirement per API 521/520.' },
    { icon: 'ðŸ”¬', text: 'NACE MR0175 specifies material requirements for Hâ‚‚S sour service to prevent sulfide stress cracking (SSC) in oil & gas equipment.' },
    { icon: 'ðŸŒ¡ï¸', text: 'LTCS (Low Temperature Carbon Steel) is mandatory below âˆ’29 Â°C to prevent brittle fracture â€” critical for LNG and LPG services.' },
    { icon: 'ðŸ“', text: 'API 6D requires trunnion-mounted ball valves on NPS â‰¥ 6â€³ isolating duty to prevent excessive stem torque and seat deformation.' },
    { icon: 'âš¡', text: 'ISA-5.1 standardises instrument identification â€” a common language for engineers, operators, and inspectors worldwide.' },
    { icon: 'ðŸ”§', text: 'OS&Y (Outside Screw and Yoke) annotation is mandatory on gate valves NPS â‰¥ 2â€³ per API 600 for visible open/closed status.' },
    { icon: 'ðŸ“Š', text: 'A complete control loop on a P&ID must show: process element â†’ transmitter â†’ controller â†’ final control element.' },
    { icon: 'ðŸ’§', text: 'Gravity-drain lines require slope annotation â‰¥ 1:100 and low-point drain valves to ensure full drainage during maintenance.' },
    { icon: 'ðŸ­', text: 'AIFlow\'s 8-pass AI engine checks 40+ engineering compliance domains per drawing â€” covering instruments, piping, valves, and safety.' },
    { icon: 'âš ï¸', text: 'Specification-break symbols (filled triangle) are mandatory at every pipe-class transition to avoid construction ambiguity.' },
    { icon: 'ðŸ”´', text: 'Critical Stress Lines require anchor points (â–³), line guides, and expansion loops when operating above 120 Â°C.' },
  ];
  ...
  // Derived: which pass is currently active based on overall progress (soft-coded â€“ tied to PASS_STAGES length)
  ...
  const handleImportLineList = async (projectId, projectName) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: `Line list imported from DesignIQ â€” ${lineListData.length} lines loaded` });
  ...
  const handleImportCSSList = async (projectId, projectName) => {
    ...
    try {
      ...
      setImportedCSSLabel(`DesignIQ: ${projectName} â€” Critical Stress (${cssData.length} lines)`);
      setShowCSSDesignIQModal(false);
      setMessage({ type: 'success', text: `Critical stress line list imported â€” ${cssData.length} lines loaded` });
      setTimeout(() => setMessage({ type: '', text: '' }), 4000);
    } catch (err) {
    ...
    try {
      ...
      if (err.response?.status === 401) {
        ...
        if (backendError.includes('OpenAI') || backendError.includes('API key')) {
          errorMessage = 'âš ï¸ Configuration Error: OpenAI service is not configured on the server. Please contact the administrator.';
        } else if (backendError.includes('quota') || backendError.includes('credits')) {
          errorMessage = 'âš ï¸ Service Limit: Analysis service has reached its usage limit. Please contact the administrator.';
